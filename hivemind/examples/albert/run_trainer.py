...
import utils
from tokenize_wikitext103 import tokenize_function
...
def main():
    ...
    try:
        ...
        logger.warning(
            f"No tokenizer data found in {dataset_args.tokenizer_path}, downloading from {dataset_args.config_path}"
        )
        Path(dataset_args.tokenizer_path).mkdir(parents=True, exist_ok=True)
        tokenizer = AlbertTokenizerFast.from_pretrained(dataset_args.config_path, cache_dir=dataset_args.cache_dir)
        tokenizer.save_pretrained(dataset_args.tokenizer_path)
        logger.info(f"Saved tokenizer to {dataset_args.tokenizer_path}")
    ...
    try:
        ...
        logger.warning(
            f"No tokenized dataset found in {dataset_args.dataset_path}, downloading and preprocessing wikitext-103..."
        )
        Path(dataset_args.dataset_path).mkdir(parents=True, exist_ok=True)
        ...
        tokenized_datasets.save_to_disk(dataset_args.dataset_path)
        logger.info(f"Saved tokenized dataset to {dataset_args.dataset_path}")
